...
SELECT EMP_ID, EMP_NAME, EMP_NO FROM EMPLOYEE; -- 특정 컬럼 조회

SELECT * FROM EMPLOYEE; -- 테이블 전부

SELECT * FROM EMPLOYEE
--WHERE EMP_ID = '200'; -- 해당 값이 있는 컬럼
 ORDER BY EMP_ID ASC; -- 오름차순
--ORDER BY EMP_ID DESC; -- 내림차순
-- 데이터를 조회한 결과를 RESULT SET 이라고 함
-- RESULT SET : SELECT 구문에 의해 반환된 행들의 집합
-- RESULT SET은 0개 이상의 행이 포함될 수 잇음
...
, SALARY*12+SALARY*NVL(BUNUS,0) " 총수령액"
, (SALARY*12*SALARY*NVL(BONUS,0)) - (SALARY*0.03) AS "실수령액"
FROM EMPLOYEE;
--SELECT EMP_NAME,SALARY*12,SALARY(SALARY*12(BONUS,0)) - (SALARY*0.03)
--FROM EMPLOYEE;
...
SELECT EMP_NAME AS 이름,SALARY AS 월급,NVL(BONUS,0)
...
SELECT  EMP_NAME
FROM EMPLOYEE
--WHERE EMP_NAME LIKE '__연'; 세글자 이름중의 연으로 끝나는 이름
WHERE EMP_NAME LIKE '%연'; -- EMP_NAME 에서 연으로 끝나는 이름
--2.EMPLOYEE 테이블에서 전화번호 처음 3자리가 010이 아닌 사원의
-- 이름, 전화번호를 출력하시오
SELECT EMP_NAME,PHONE FROM EMPLOYEE
WHERE PHONE NOT LIKE '010%'; -- PHONE 에서 010으로 시작하는 데이터
--3.EMPLOYEE 테이블에서 메일주소의 's'가 들어가면서, 
-- DEPT_CODE가 D9 또는 D6이고 
-- 고용일이 90/01/01 ~ 00/12/01이면서, 
-- 월급이 270만원이상인 사원의 전체 정보를 출력하시오
SELECT * FROM EMPLOYEE
WHERE EMAIL LIKE '%s%'  -- s를 포함하고
--AND (DEPT_CODE = 'D9' OR DEPT_CORD = 'D6')
AND (DEPT_CODE IN ('D9','D6')) -- D9 또는 D6을 포함하고
...
--(퇴사 여부 : ENT_YN)
SELECT EMP_NAME AS 이름,EMP_ID 
AS 부서코드,DEPT_CODE AS 고용일,ROUND(ENT_DATE - HIRE_DATE) AS 근무기간
,HIRE_DATE AS 입사일,ENT_DATE AS 퇴직일
FROM EMPLOYEE
WHERE ENT_YN LIKE 'Y';
...
SELECT EMP_NAME,SALARY,FLOOR(FLOOR(SYSDATE-HIRE_DATE)/1000)*SALARY*0.1 AS 특별보너스
FROM EMPLOYEE
