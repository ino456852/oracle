...
-- 자동적으로 일련의 동작(Operation) 처리가 수행되도록 하는 데이터 베이스 객체중 하나임
-- 예시) 회원 탈퇴가 이루어진 경우 탈퇴한 회원 정보를 일정기간 저장해야 하는 경우
...
VALUES((SELECT MAX(EMP_ID)+1 FROM EMPLOYEE), '이용자','000624-3233444','01022224444','J5','S5');
-- 트리거 생성 후 저장된 테이블에 명령어를 실행하면 트리거가 동작됨
-- 신입사원이 입사하였습니다.
-- 1 행 이(가) 삽입되었습니다.
...
FOR EACH ROW
BEGIN
    DBMS_OUTPUT.PUT_LINE('신입 사원이 입사하였습니다.');
END;
/

-- 예제2 EMPLOYEE 테이블에 급여 정보가 변경되면 전후 정보를 출력하는 트리거를 생성하시오
...
FOR EACH ROW
BEGIN
    IF(:OLD.SALARY != :NEW.SALARY) -- EMPLOYEE 테이블에 무조건 UPDATE가 되면 실행 되는것을 방지
    THEN
...
(
    ...
    PCODE NUMBER, --CONSTRAINT FK_PRODUCT_ID  RFOREIGN KEY(,
...
BEGIN 
    IF :NEW.STATUS = '입고'
    THEN
        -- INSERT INTO PRODUCT_IO VALUSE(1, 1, SYSDATE, 10, '입고');
            UPDATE PRODUCT SET STOCK = STOCK + :NEW.AMOUNT WHERE PCODE = :NEW.PCODE;
    ELSIF :NEW.STATUS = '출고'
    THEN
        -- INSERT INTO PRODUCT_IO VALUSE(1,1,SYSDATE,5,'출고');
            UPDATE PRODUCT SET STOCK = STOCK - :NEW.AMOUNT WHERE PCODE = :NEW.PCODE;
    END IF;
END;
/
...
VALUES(SEQ_PRODUCT_PCODE.NEXTVAL,1,SYSDATE,10,'입고');
INSERT INTO PRODUCT_IO 
