...
DECLARE
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE('이름 : ' || EMPID.EMP_NAME || ', 급여 : ' || EMPID.SALARY);
        
    END;
    /
        
-- PL/SQL 의 IF문
-- IF(조건식) THEN (실행문) END_IF;
-- IF(조건식) THEN (실행문) ELSE (실행문2) END_IF;
-- IF(조건식) THEN (실행문1) ELSIF(조건식2) THEN (실행문)
-- ELSIF(조건식3) THEN (실행문) ELSE(실행문);
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('급여 등급 : ' || SLEVEL);
END;
/
...
BEGIN 
    ...
    CASE INPUT
        ...
        WHEN 2 THEN DBMS_OUTPUT.PUT_LINE('노간색입니다.');
        WHEN 3 THEN DBMS_OUTPUT.PUT_LINE('초간색입니다.');
    END CASE;
END;
/

-- 급여등급 출력문을 CASE로 바꿔보기
DECLARE    
    EMPID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    SALARY EMPLOYEE.SALARY%TYPE;
...
BEGIN
    ...
    CASE SALARY
        ...
        WHEN 3 THEN SLEVEL := 'B';
        WHEN 4 THEN SLEVEL := 'C';
        ELSE SLEVEL := 'A';
    END CASE;
    DBMS_OUTPUT.PUT_LINE('사번 : ' || EMPID);
    DBMS_OUTPUT.PUT_LINE('이름 : ' || ENAME);
    DBMS_OUTPUT.PUT_LINE('급여 : ' || SAL*1000000);
    DBMS_OUTPUT.PUT_LINE('급여 등급 : ' || SLEVEL);
END;
/
...
-- WHILE LOOP
-- 제어 조건자 TRUE인 동안만 문장이 반복 실행됨
-- LOOP를 실행할 때 조건이 처음부터 FALSE면 한번도 수행되지 않을 수 있음
...
BEGIN
    ...
    LOOP
        N := N + 1;
        CONTINUE WHEN MOD(N,2) = 0; -- 나머지가 00이면 짝수니까 출력하지 않고 반복으로
        DBMS_OUTPUT.PUT_LINE(N);
    END LOOP;
END;
/
...
-- @실습문제4
-- EMPLOYEE 테이블의 사번이 200 ~ 210인 직원들의 
-- 사원번호, 사원명, 이메일을 출력하시오~!
DECLARE
...
END;
/
SELECT NO,INPUT_DATE FROM KH_NUMBER_TBL;
ROLLBACK;
...
-- PL/SQL WHEN NO_DATE, THEN DBMS_OUTPUT.PUT_LINE('노 데이터!');
-- NO_DATA_FOUND
