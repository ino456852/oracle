...
-- 프로시저(PROCEDUAL), 함수(FUNTION), 트리거(TRIGER),동의어(SYSNONYN)
-- 커서(CURSOR),패키지(PACKAGE) ...

-- 3. ROLE
-- 3.1 개요
-- - 사용자에게 여러 개의 권한을 한번에 부여할 수 있는 데이터 베이스 객체
-- - 사용자에게 권한을 부여할 때 한개씩 부여하게 된다면 권한 부여및 함수의 권리가
-- 불편하므로 사용하는 객체임
...
-- * DCL
-- 권한 부여 및 회수는 관리자 섹션(빨간색)에서 사용 가능
-- 관리자 계정
-- 1. SYSTEM : 일반 관리자, 대부분의 권한울 가지고 있음.
-- 2. SYS    : DB생성/삭제 권한이 있음. 로그인 옵션으로 AS SYSDB 적어줘야함
GRANT CONNECT , RESOURCE TO KH;
-- CONNECT,RESOURCE TO 는 권한이 모여있는 ROLE이다
-- ROLE에 부여된 시스템 권한 확인

SELECT * FROM DBA_SYS_PRIVS WHERE GRANTEE = 'CONNECT'; -- 관리자 계정에서 사용
SELECT * FROM DBA_SYS_PRIVS WHERE GRANTEE = 'RESOURCE'; -- 관리자 계정에서 사용
-- CREATE VIEW 권한이 RESOURCE ROLE 에 없었기 때문에 따로 권한 부여를 해줌
...
-- - KEY와 VALUE형태로 생성이 되면 KEY애는 인덱스로 만들 컬럼값,VALUE에는 행이 저장됨
-- 주소값이 저장됨
-- - 장점 : 검색속도가 빨라지고 시스템에 걸리는 부하를 줄여서 시스템 전체 성능을 향상 시킬수 있음
-- - 단점 : 인덱스를 위한 추가 저장 공간이 필요하고 인덱스를 생성하는데 시간이 걸림
-- 데이터의 변경작업이 자주 일어나는 테이블에 INDEX 생성시 오히려 성능저하가 발생할 수 있음
-- - 어떤 컬럼에 인덱스를 만들면 좋을까?
-- 데이터 값이 중복된 것이 없는 고유한 데이터값을 가지고 컬럼에 만드는 것이 가장 조흥ㅁ
-- 그리고 자주 사용되는 컬렘에 만들면 좋음! - 절대적이지 않음
-- 4.2 사용 예-- 
-- * 효율적인 인덱스 사용 예
...
WHERE EMP_ID LIKE '2%';
SET TIMING OFF;
-- F10으로 실행하여 오라클 PLAN확인 가능,튜닝시 사용됨
