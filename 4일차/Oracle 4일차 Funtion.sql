...
SELECT FLOOR(SYSDATE-HIRE_DATE) FROM EMPLOYEE; -- 단일행 함수
...
--      E TRIM
--      F SUBSTR                문자열 잘라줌 2개 연결
--      G CONCAT                문자열 합쳐줌 3개이상 연결 가능
--      H REPLACE               문자열 바꿔줌
--      사용법은 함수명(값1, 값2 ... ) : 값에는 리터럴이나 컬럼명을 넣어주면 됨

-- 1.1.3 날짜 처리 함수
-- ADDMONTHS , MONTHSBETWEEN, LAST_DAY,EXTRAT,SYSDATE
-- @실습문제
/*
...
*/
SELECT SYSDATE "입대날짜",
ADD_MONTHS(SYSDATE,18) "제대일자", -- 오늘 날짜에 18개월 플러스
...
--	ex) 송은희	    1963  	    0
SELECT EMP_NAME "직원명",
'19'||SUBSTR(EMP_NO,1,2)"년생",NVL(BONUS,0)"보너스"
FROM EMPLOYEE
WHERE EMP_NO LIKE '6%';
...
SELECT TO_NUMBER ('500') + 300 FROM DUAL; -- 자동 형변환
-- 자동 형변환 안돼는 경우
SELECT '1,000,000' - '500,000' FROM DUAL; --"invalid number"
...
,TO_CHAR(HIRE_DATE,'YYYY"년 "MM"월 "DD"일"') "입사일자"
FROM EMPLOYEE;
...
--DECODE 함수
-- 사용법 : DECODE(변하는값,조건값,참일때값,조건값,참일때값,모두 아닐때 값)
-- 사용법 : DECODE(1,         1,   '빨강'    ,2,  '노랑'     ,'파랑')
-- CASE 함수
-- 사용법 : CASE WHEN 변하는값 = 조건값 THEN 참일때 값 WHEN 변하는값 = 조건값 THEN 참일때 값 ELSE 모두 아닐때 값 END
-- 예시     CASE WHEN 변하는값 = 1 THEN '빨강' WHEN 변하는값 = 2 THEN '노랑' ELSE '파랑' END
SELECT  EMP_NAME "이름",
DECODE(SUBSTR(EMP_NO,8,1) -- 변하는 값
,1,'남',2,'여',3,'남','4','여','무') "성별"
,   CASE 
...
-- 2.1.4 MAX/MIN
SELECT MAX(SALARY) "최대",MIN(SALARY) "최소" FROM EMPLOYEE;


SELECT DEPT_CODE "부서명",SUM(SALARY) 
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY 1 ASC;
...
SELECT DEPT_CODE "부서명",SUM(SALARY)"급여합계", FLOOR(AVG(SALARY))"급여평균"
,COUNT(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY 1 ASC;
...
--부서코드 순으로 정렬하세요
-- BONUS 컬럼의 값이 존재한다면 그 행을 1로 카운팅, 
-- 보너스를 지급받은 사원이 없는 부서도 있음을 확인
SELECT DEPT_CODE"부서명", COUNT(SALARY)
FROM EMPLOYEE
WHERE BONUS IS NOT NULL
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;

-- EMPLOYEE 테이블에서 직급이 J1인 
-- 사람들을 제외하고 직급별 사원수 및 평균급여를 출력하세요.
SELECT COUNT (*), FLOOR(AVG (SALARY))
FROM EMPLOYEE
-- WHERE JOB_CODE != 'J1'
...
-- 실습4
-- EMPLOYEE 테이블에서 직급이 J1인 사람들을 제외하고 입사년도별 
-- 인원수를 조회해서,
...
-- 그훕함수 결과이지만 GROOUP BY 를 쓰면 기준별로 결과가 나옴
...
GROUP BY DEPT_CODE, JOB_CODE; -- 부서별,직급별

-- 실습7
-- 부서내 성별 인원수를 구하시오
SELECT DEPT_CODE, DECODE(SUBSTR(EMP_NO,8,1)
,1,'남',2,'여',3,'남','4','여','무') "성별", COUNT(SALARY) FROM EMPLOYEE
GROUP BY DEPT_CODE, DECODE(SUBSTR(EMP_NO,8,1)
,1,'남',2,'여',3,'남','4','여','무')
ORDER BY 1 ASC;
...
-- 부서별 인원수가 5명 미만/초과 인 레코드를 출력하세요
SELECT DEPT_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING COUNT(*) < 5; -- GROUP BY 한 결과값
...
-- 실습문제2
--부서내 직급별 인원수가 3명이상인
-- 직급의 부서코드, 직급코드, 인원수를 출력하세요.
DESC EMPLOYEE;
SELECT DEPT_CODE "부서코드",JOB_CODE"직급코드",COUNT(*)"카운트" -- DEPT_CODE의 카운트, JOB_CODE의 카운트
FROM EMPLOYEE
GROUP BY DEPT_CODE,JOB_CODE
HAVING COUNT(*) >= 3;

-- 실습문제3
-- 매니져가 관리하는 사원이 2명이상인 매니져아이디와 관리하는 사원수를 출력
...
FROM EMPLOYEE
GROUP BY ROLLUP(DEPT_CODE,JOB_CODE) -- 부서별 월급 합계 추가
ORDER BY 1;

SELECT DEPT_CODE,JOB_CODE,SUM(SALARY)
FROM EMPLOYEE
GROUP BY CUBE(DEPT_CODE,JOB_CODE) -- 부서별 월급 합계,직급별 월급 합계 추가
ORDER BY 1;
...
SELECT LOWER('WELCOME TO THE WORLD') FROM DUAL;
SELECT UPPER('welcome to the world') FROM DUAL;