...
/*
    ...
    사용 가능하게 함)
    - SELECT 쿼리의 실행 결과를 화면에 정리한 논리적인 가상의 테이블
    (실질적인 데이터를 저장하고 있지 않지만 하나의 테이블 사용 가능함)
...
--    1.2.2 Inline View(저장x) - FROM절 뒤에 적는 서브쿼리
    SELECT * 
    FROM (SELECT EMP_ID,DEPT_CODE,JOB_CODE,MANAGER_ID FROM EMPLOYEE);
...
UPDATE V_EMPLOYEE -- VIEW를 바꾸면 원본도 바뀜
SET DEPT_CODE = 'D8'
WHERE EMP_ID = '200';
...
-- 3. DBA_XXX : 데이터베이스 관리자만 접근이 가능함 객체 등의 정보 조회
...
-- 2.2 Sequence 옵션
-- 2.2.1 MINVALUE : 발생시길 최소값 지정
-- 2.2.2 MAXVALUE : 발생시길 최대값 지정
-- 2.2.3 START WITH : 처음 발생시길 시점값 지정, 기본값 1
-- 2.2.4 INCREMENT BY : 다음 값에 대한 증가치, 기본값 1
-- 2.2.5 NOCYCLE : 시퀀스 값이 최대값까지 증가를 완료하면 CYCLE은 START WITH로 다시 시작함
-- 2.2.6 NOCACHE : 메모리 상에서 시퀀스 값을 관리, 기본값 20. NOCACHE 안하면 갑자기 시퀀스 값이 증가하게 됨
...
SELECT SEQ_KH_USER_NO.NEXTVAL FROM DUAL; -- 첫 NEXTVAL은 시퀀스를 계시함 밀수조건
...
SELECT SEQ_KH_USER_NO.CURRVAL FROM USER_TCL;
...
COMMENT ON COLUMN ORDER_TBL.USER_ID IS '고개아이디';
COMMENT ON COLUMN ORDER_TBL.PRODUCT_ID IS '주문상품아이디';
...
CREATE SEQUENCE SEQ_KH_MEMBER
START WITH 500
INCREMENT BY 10
NOCYCLE
NOCACHE;

CREATE SEQUENCE SEQ_KH_MEMBER_JOIN
NOCYCLE
NOCACHE;


INSERT INTO KH_MEMBER
VALUES(SEQ_KH_MEMBER.NEXTVAL,'홍길동',20,SEQ_KH_MEMBER_JOIN.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_KH_MEMBER.NEXTVAL,'청길동',30,SEQ_KH_MEMBER_JOIN.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_KH_MEMBER.NEXTVAL,'외길동',40,SEQ_KH_MEMBER_JOIN.NEXTVAL);
INSERT INTO KH_MEMBER
VALUES(SEQ_KH_MEMBER.NEXTVAL,'고길동',50,SEQ_KH_MEMBER_JOIN.NEXTVAL);

SELECT * FROM KH_MEMBER;

DROP SEQUENCE SEQ_KH_MEMBER;
...
ALTER SEQUENCE SEQ_KH_MEMBER_JOIN
-- START WITH 변경 불가 -> 삭제후 다시 생성
INCREMENT BY 1
MAXVALUE 100000;
